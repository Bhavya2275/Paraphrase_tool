...
`;
        ...
        chrome.storage.sync.get(["geminiApiKey"], async (result) => {
          if (chrome.runtime.lastError) {
            ...
            try {
              ...
              const decryptedKey = await decryptText(
                storedKey,
                "spellAI_master_key"
              );
  ...
  async function handleActionReplace(action, text, target) {
    ...
    console.log("[spellai] handleActionReplace called:", {
      action,
      text,
      target,
    }); // Debug log
    ...
    try {
      ...
      if (!apiKey) {
        ...
        showModal(
          "Gemini API key not set. Please set it in the extension popup."
        );
        removeMenu();
        removeCursorSpinner();
        window.spellaiIsProcessing = false;
  ...
  function createMenu(x, y) {
    ...
    const tones = [
      {
        key: "grammar",
        label: "Grammar",
        icon: `<img src="${chrome.runtime.getURL(
          "Grammar.png"
        )}" alt="Grammar Icon" class="menu-icon">`,
      },
      {
        key: "professional",
        label: "Professional",
        icon: `<img src="${chrome.runtime.getURL(
          "professional.png"
        )}" alt="Professional Icon" class="menu-icon">`,
      },
      {
        key: "humanize",
        label: "Humanize",
        icon: `<img src="${chrome.runtime.getURL(
          "Humanize.png"
        )}" alt="Humanize Icon" class="menu-icon">`,
      },
    ];
    function showDropdown(e) {
      ...
      tones.forEach((tone) => {
        ...
        hoverLabel.style.pointerEvents = "none";
        hoverLabel.style.transition = "opacity 0.18s 0.3s";
        toneBtn.onmouseenter = () => {
          hoverLabel.style.opacity = "1";
        };
        toneBtn.onmouseleave = () => {
          hoverLabel.style.opacity = "0";
        };
    ...
    if (
      lastTarget &&
      (lastTarget.tagName === "INPUT" || lastTarget.tagName === "TEXTAREA")
    ) {
      // Always use the endpoint of the selection
      const endpoint = Math.max(
        lastTarget.selectionStart,
  ...
  function showAskModal(selectedText, target) {
    ...
    selectedPreview.className = "selected-text-preview";
    ...
    textarea.placeholder =
      "e.g., write an email, improve this text, explain something, translate, or write a reply...";
    textarea.className = "ask-modal-textarea";
    box.appendChild(textarea);

    const generateBtn = document.createElement("button");
    generateBtn.textContent = "Generate";
    generateBtn.className = "ask-modal-button";
    ...
    function attemptFocus() {
      ...
      try {
        ...
        if (document.activeElement === textarea) {
          focusSuccess = true;
          console.log("[SpellAI] Focus successful");
          return true;
        }

        return false;
      } catch (error) {
        console.log("[SpellAI] Focus error:", error);
        return false;
      }
    }

    // Strategy 1: Immediate focus after DOM insertion
    ...
    // Strategy 2: Focus on first user interaction
    const focusOnInteraction = function (e) {
      if (!focusSuccess) {
    ...
    overlay.addEventListener("mousedown", focusOnInteraction, { once: true });
    overlay.addEventListener("touchstart", focusOnInteraction, { once: true });
    box.addEventListener("mousedown", focusOnInteraction, { once: true });
    box.addEventListener("touchstart", focusOnInteraction, { once: true });

    // Strategy 3: Focus when modal becomes visible (for sites with strict policies)
    const observer = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting && !focusSuccess) {
          setTimeout(() => {
            attemptFocus();
          }, 100);
        }
    ...
    // Strategy 4: Fallback focus on textarea click
    ...
    // Strategy 5: Focus on modal visibility change (for LinkedIn-like sites)
    ...
    generateBtn.onclick = async function (e) {
      ...
      if (!query) {
        // Ensure focus when user tries to generate without text
        attemptFocus();
        return;
      }
      ...
      try {
        ...
        if (!apiKey) {
          suggestionBox.innerHTML =
            '<div class="error-message">Gemini API key not set. Please set it in the extension popup.</div>';
          return;
        }
...
`;
        ...
        if (result) {
          ...
          applyBtn.onclick = function (ev) {
            ...
            if (
              !insertTarget ||
              !(
                insertTarget.tagName === "INPUT" ||
                insertTarget.tagName === "TEXTAREA" ||
                insertTarget.isContentEditable
              )
            ) {
          ...
          suggestionBox.innerHTML =
            '<div class="error-message">No suggestions found. Please try a different query.</div>';
        }
      } catch (err) {
        suggestionBox.innerHTML =
          '<div class="error-message">Network or extension error: ' +
    ...
    // Enhanced enter key handling for textarea
  ...
  //     console.log(
  //       "steps [askOverlay] selection inside Ask modal, removing menu"
  //     );
  //     removeMenu();
  //     return;
  //   }
  ...
  //     let coords = null;
  //     if (isInput) {
  //       console.log("steps [input/textarea] detected", {
  //         selectionStart: target.selectionStart,
  //         selectionEnd: target.selectionEnd,
  //       });
  ...
  //         showMenu = true;
  //         console.log(
  //           "steps [input/textarea] valid selection, coords:",
  //           coords
  //         );
  //       } else {
  //         console.log(
  //           "steps [input/textarea] no selection, menu will be removed"
  //         );
  //       }
  //     } else if (isEditable) {
  ...
  //           showMenu = true;
  //           console.log(
  //             "steps [contenteditable] valid selection, coords:",
  //             coords
  //           );
  //         } else {
  //           console.log(
  //             "steps [contenteditable] no valid rect, menu will be removed"
  //           );
  //         }
  //       } else {
  //         console.log(
  //           "steps [contenteditable] no selection, menu will be removed"
  //         );
  //       }
  //     } else {
  ...
  //         if (rect) {
  //           if (!insideField) {
  //             console.log("steps [createMenu] called for field, coords:", {
  //               x: rect.left,
  //               y: rect.bottom,
  //             });
  ...
  //         } else {
  //           console.log(
  //             "steps [general selection] no valid rect, menu will be removed"
  //           );
  //         }
  //       } else {
  //         console.log(
  //           "steps [general selection] no selection, menu will be removed"
  //         );
  //       }
  //     }
  //     if (showMenu && coords) {
  ...
  function showMenuIfSelection(e) {
    ...
    // Function to detect if target is a search bar
    function isSearchBar(element) {
      ...
      const searchIndicators = ["search", "query", "find", "filter", "lookup"];

      // Check name attribute
      if (element.name) {
        const name = element.name.toLowerCase();
        if (searchIndicators.some((indicator) => name.includes(indicator))) {
          return true;
        }
      }
      ...
      if (element.id) {
        ...
        if (searchIndicators.some((indicator) => id.includes(indicator))) {
          return true;
        }
      }
      ...
      if (element.placeholder) {
        ...
        if (
          searchIndicators.some((indicator) => placeholder.includes(indicator))
        ) {
          return true;
        }
      }
      ...
      if (element.className) {
        ...
        if (
          searchIndicators.some((indicator) => className.includes(indicator))
        ) {
          return true;
        }
      }
      ...
      if (element.getAttribute("aria-label")) {
        ...
        if (
          searchIndicators.some((indicator) => ariaLabel.includes(indicator))
        ) {
          return true;
        }
      }

      // Check if parent container suggests search functionality
      let parent = element.parentElement;
      while (parent && parent !== document.body) {
        if (parent.className) {
          const parentClass = parent.className.toLowerCase();
          if (
            searchIndicators.some((indicator) =>
        ...
        if (parent.id) {
          ...
          if (
            searchIndicators.some((indicator) => parentId.includes(indicator))
          ) {
            return true;
          }
        }
    ...
    // Check if it's a search bar and exit early
    if (isInput && isSearchBar(target)) {
      console.log("steps [search bar] detected, removing menu");
      removeMenu();
      return;
    }
    ...
    setTimeout(() => {
      ...
      if (isInput) {
        ...
        if (sel && sel.rangeCount > 0 && !sel.isCollapsed) {
          ...
          while (node) {
            if (node.nodeType === 1) {
              ...
              if (
                ...
                insideField = true;
                // Check if this field is a search bar
                if (isSearchBar(node)) {
                  insideSearchBar = true;
                }
                break;
              }
            }
          ...
          // If inside a search bar, don't create menu
          if (insideSearchBar) {
            console.log(
              "steps [general selection] inside search bar, removing menu"
            );
            removeMenu();
            return;
          }
  ...
  if (chrome.storage && chrome.storage.onChanged) {
    chrome.storage.onChanged.addListener((changes, area) => {
      if (
        area === "sync" &&
        (changes.spellaiMode || changes.spellaiShortcuts)
      ) {
        loadSpellaiSettings();
      }
    });
  ...
  document.addEventListener(
    ...
    async function (ev) {
      ...
      for (const action in spellaiShortcuts) {
        if (
          spellaiShortcuts[action].replace(/\s+/g, "").toUpperCase() ===
          pressed.replace(/\s+/g, "").toUpperCase()
        ) {
          matched = action;
          break;
        }
  ...
  function showMenuIfSelectionPatched(e) {
    ...
    // Helper function to check if element is in contenteditable
    ...
    // Function to find if selection is within an input context
    function isSelectionInInputContext() {
      ...
      // Find the closest input/textarea to the selection start
      let element =
    ...
    // Check immediate context from event target
    const target = e && e.target;
    const targetIsInput =
      (target && target.tagName === "INPUT" && target.type === "text") ||
      (target && target.tagName === "TEXTAREA");
    const targetIsEditable = isInContentEditable(target);

    // Check current selection context
    const selectionInInput = isSelectionInInputContext();

    // Check if currently focused element is an input
    const activeElement = document.activeElement;
    const activeIsInput = isInputElement(activeElement);

    // Determine if we're in an input context
    const isInInputContext =
      targetIsInput || targetIsEditable || selectionInInput || activeIsInput;

    console.log("Target is input:", targetIsInput);
    console.log("Target is editable:", targetIsEditable);
    console.log("Selection in input:", selectionInInput);
    console.log("Active element is input:", activeIsInput);
    console.log("Final isInInputContext:", isInInputContext);

    // Only proceed if we're in an input context
    if (!isInInputContext) {
      removeMenu();
      return;
    }

    // Call the original function if we're in an input context
